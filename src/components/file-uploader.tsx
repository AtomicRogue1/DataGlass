...
function FileUploaderDropdown({ onDataParsed }: { onDataParsed: (data: Record<string, string>[]) => void }) {
...
export default function FileUploader() {
  const { setCsvData, setHasData, initialDataProcessingDone, setInitialDataProcessingDone} = useData();
  ...
  const handleDataParsed = useCallback((data: Record<string, string>[]) => {
    ...
    setTimeout(() => {
      ...
      setInitialDataProcessingDone(false);
    }, 500);
